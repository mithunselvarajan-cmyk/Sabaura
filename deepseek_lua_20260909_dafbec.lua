-- ============================================================
-- RUSTYHUB AFK SERVER HOPPER + WEBHOOK + STATUS GUI
-- ============================================================
...
local HOP_INTERVAL = 120

-- ============================================================
-- STATUS GUI VARIABLES (updated by the script)
-- ============================================================
local status = {
    running = true,
    petsFound = 0,
    nextHopIn = HOP_INTERVAL,
    lastScanTime = os.date("%H:%M:%S"),
...
local function createGUI()
    ...
    local old = pg:FindFirstChild("RustyhubStatus")
    if old then old:Destroy() end

    local sg = Instance.new("ScreenGui")
    sg.Name = "RustyhubStatus"
    ...
    local labelConfigs = {
        ...
        { name = "NextHop", text = "Next hop in: --s" },
    ...
    rejoinBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            serverHop()  -- defined below
        end)
    end)

    -- Return the label objects for updates
    return labels, rejoinBtn
end

-- We'll update these later
local guiLabels = nil
local guiRejoinBtn = nil
...
local function updateGUI()
    ...
    guiLabels.Status.Text = status.running and "● Running" or "● Stopped"
    guiLabels.Pets.Text = "Pets ≥30M: " .. status.petsFound
    guiLabels.NextHop.Text = "Next hop in: " .. math.ceil(status.nextHopIn) .. "s"
    guiLabels.LastScan.Text = "Last scan: " .. status.lastScanTime
end
...
local function sendWebhook()
    ...
    status.petsFound = #pets
    status.lastScanTime = os.date("%H:%M:%S")
    updateGUI()
    ...
    print("[Webhook] Sent! Server: " .. jobId .. " | Pets: " .. #pets)
end
...
-- ============================================================
-- TRIGGER SCAN ON PLAYER JOIN
-- ============================================================
local function onPlayerAdded()
    print("[Trigger] New player joined, scanning for pets...")
    task.wait(2)
    sendWebhook()
end

-- ============================================================
-- MAIN LOOP (auto hop every 2 minutes)
-- ============================================================
local function main()
