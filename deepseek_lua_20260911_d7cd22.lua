...
local BOT_TOKEN = "MTU0NzkzMDUzMzYyMTgwMTAyMQ.G2o1eM.jms9vH2pY1nVn2b4Fg3cWVKC08ILy-4aAbhkNM"
...
local function extractTop(text)
    local name = text:match("Top:%s*(.-)%s*%-%s*([%d%.]+[KMBT]?)/s")
    local jobId = text:match("gameInstanceId=([%x%-]+)")
    local placeId = text:match("placeId=(%d+)")
    if not jobId then return nil end
    if not name then return nil end

    local mps = text:match("Top:%s*.-%-%s*([%d%.]+[KMBT]?)/s")
    local mutation = text:match("Top:%s*.-%-%s*[%d%.]+[KMBT]?/s%s*|%s*([^\n]+)")
