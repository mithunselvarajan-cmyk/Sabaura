...
local REJOIN_URL = "https://raw.githubusercontent.com/mithunselvarajan-cmyk/Sabaura/refs/heads/main/deepseek_lua_20260910_7f8360.lua"
                    ...
                    if type(animalData) == "table" then
                        ...
                        if name then
                            ...
                            if info and info.Generation then
                                ...
                                -- Traits: ADDITIVE (sum of bonuses)
                                if animalData.Traits and type(animalData.Traits) == "table" and TraitsData then
                                    ...
                                    for _, tr in ipairs(animalData.Traits) do
                                        ...
                                        if t and t.MultiplierModifier then
                                            traitSum = traitSum + t.MultiplierModifier
                                            hasTraits = true
                                        end
                                    end
                                    if hasTraits then
...
-- ── WEBHOOK 1: Pets ──
local function sendPetsWebhook(pets)
    local top5 = {}
...
-- ── WEBHOOK 2: Link + Top Pet ──
local function sendLinkWebhook(topPet)
    local jobId = game.JobId
    local joinLink = buildJoinLink(jobId)

    local mutPart = (topPet.mutation and topPet.mutation ~= "None" and " | " .. topPet.mutation) or ""
    local header = string.format(
        "Top: %s - %s%s\nServer link:",
        topPet.name,
        formatMPS(topPet.mps),
        mutPart
    )
    ...
    print("[Link] Sent join link: " .. joinLink)
end

-- ── COMBINED (only fires when 30M+ pets found) ──
local function sendWebhook()
    local pets = getAllPets()
    lblPets.Text = "Pets 30M+: " .. #pets
    lblScan.Text = "Last scan: " .. os.date("%H:%M:%S")

    -- 🔴 Only fire webhooks if a 30M+ pet was actually detected
    if #pets == 0 then
        print("[Detector] No pets 30M+. Staying silent.")
        return
    end

    sendPetsWebhook(pets)
    sendLinkWebhook(pets[1])
end
